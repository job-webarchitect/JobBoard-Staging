...
class Auth extends CI_Controller
{
	...
	function noaccess()
	{
			...
			$data['content'] = 'auth/access-denied';
...
/*	function login()
	...
	public function login() {
			...
			if ($this->form_validation->run() == FALSE) {
					if(isset($this->session->userdata['logged_in'])){
						...
						$result = $this->Mdl_config->login($data);
						if ($result == TRUE) {

								$username = $this->input->post('username');
								$result = $this->Mdl_config->read_user_information($username);
	...
	function register() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data['title'] = 'Register';
			$data['content']='auth/master_register';
			$this->load->view('admin/templates/login_layout',$data);
		} 
			...
			$data = array(
				...
				'admin_email' => $this->input->post('email_value'),
				'admin_pass' => $this->input->post('password')
			);


			$result = $this->Mdl_config->registration_insert($data);
			if ($result == TRUE) {
		 ...
		 elseif (!$this->config->item('allow_registration', 'tank_auth')) {	// registration is off
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
						...
						$this->session->set_flashdata('master', 'Approved successfully'); 
	...
	function registerpost() {
		 ...
		 $result = $this->Mdl_config->validate();
		 if(!$result){
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->forgot_password(
				    ...
				    $this->session->set_flashdata('forgotpass','auth_message_new_password_sent'); 
					redirect('admin/login');  
